function [rectx,recty,strip] = Strip(x,y,width)

% will need a 2x2 rotation matrix through an angle theta
Rmat = @(theta) [cos(theta) sin(theta);-sin(theta) cos(theta)];

% get the angle of each edge of the AB line.
...
%         旋转前后矩形的形状不变化，即长度，面积都不变
%         bc = distance(tem_rect(2,1),tem_rect(2,2),tem_rect(3,1),tem_rect(3,2));
